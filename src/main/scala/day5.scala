...
object day5 extends App {
  val input: List[String] = Source.fromFile("inputs/day5.txt").mkString.split('\n').toList
  //Using ListBuffer here is several orders of magnitude slower
  val instructions: ArrayBuffer[Int] = (input map { n => n.toInt }).to[ArrayBuffer]

  def move(position: Int, stepsTaken: Int): Int = {
    val newStepsTaken = stepsTaken + 1

    val offset = instructions(position)
    if (offset >= 3) instructions(position) -= 1
    else instructions(position) += 1

    val newPosition = position + offset
    if (instructions.isDefinedAt(newPosition)) move(newPosition, newStepsTaken)
    else newStepsTaken
  }

  println("Total steps taken: " + move(0, 0))
}
