...
object day6 extends App {
  var banks: ListBuffer[Int] = (inputDay6.input.split('\t') map {n => n.toInt}).to[ListBuffer]
  var seenConfigs: ListBuffer[List[Int]] = new ListBuffer()
  seenConfigs append banks.toList

  def redistribute(numDistributions: Int): Int = {
    val max = banks.max
    val indexOfMax = banks.indexOf(max)
    banks(indexOfMax) = 0
    var indexToAdd = indexOfMax + 1
    0 until max foreach { _ =>
      indexToAdd = if (banks.isDefinedAt(indexToAdd)) indexToAdd else 0
      banks(indexToAdd) += 1
      indexToAdd += 1
    }
    if (seenConfigs.contains(banks.toList)) {
      println("Size of loop: " + (seenConfigs.length - seenConfigs.indexOf(banks.toList)))
      numDistributions + 1
    }
    else {
      seenConfigs append banks.toList
      redistribute(numDistributions + 1)
    }
  }

  println("Number of redistributions: " + redistribute(0))
}
